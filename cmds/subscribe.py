...
class ActionCancel(Exception): pass

guildID = [719132687897591808, 669934356172636199]

tz = datetime.timezone(datetime.timedelta(hours=8))
def get_time() -> datetime.date: return datetime.datetime.now(tz).date()
...
def managerCheck(ctx) -> bool:
   return channelCheck(ctx) and ctx.author.id in [590430031281651722, 384233645621248011, 546614210243854337]
...
def date_valid(ctx, date):
  try:  date = datetime.date.fromisoformat(date)
  except:  raise DateInvaild('日期格式不合理')
  else:
...
class SUBSCRIBE(CogInit):
  ...
  async def actionCheck(self, msg, author):
    ...
    def check(reaction, user):
      if reaction.message.id == msg.id and user == author:
        if str(reaction.emoji) == "\N{WHITE HEAVY CHECK MARK}": return True
        elif str(reaction.emoji) == "\N{CROSS MARK}": raise ActionCancel
  ...
  async def subscribe(self, ctx, subscriber, artist, mark=""):
    if managerCheck(ctx):
      subscriber = subscriber.mention.replace('!', '')
      if SQL_getData("artists", "artist", artist):  await ctx.send(f"繪師 `{artist}` 已有訂閱紀錄", delete_after = 5)  #已訂閱繪師
      elif not SQL_getData("subscribers", "subscriber", subscriber):  await ctx.send(f"{subscriber} 不在訂閱者名單內", delete_after = 5)  
      #無此訂閱者
      else:
        msg = await ctx.send(f"請確認 {subscriber} 訂閱 `{artist}`")
        if not await self.actionCheck(msg, ctx.author):  await ctx.send("新增資料取消", delete_after = 5)
        else:
  ...
  async def unsubscribe(self, ctx, artist):
    if managerCheck(ctx):
      ...
      if artist:
        ...
        if not await self.actionCheck(msg, ctx.author):   await ctx.send("刪除資料取消", delete_after = 5)
        else:
    ...
    if managerCheck(ctx):
      ...
      if SQL_getData('subscribers', 'subscriber', newsubscriber) and artist:
        if artist[1] == newsubscriber:  #原訂閱者和新訂閱者相同
          ...
          if not await self.actionCheck(msg, ctx.author):  await ctx.send("更改訂閱者取消", delete_after = 5)
          else:
  ...
  async def update(self, ctx, artists, timestamp = None):
    ...
    if timestamp == None:  timestamp = str(get_time())
    else:  timestamp = f"{get_time().year}-{timestamp}"
    
    try:  date_valid(ctx, timestamp)
    except DateInvaild as error:  await ctx.send(error) 
    else:
    ...
    if channelCheck(ctx):
      if target_type == 0:  #訂閱者
        ...
        if artist:
          ...
          if artist[4] == 1:  message += '新增訂閱資料後未更新\n'
          elif get_time() - artist[2] >= datetime.timedelta(days = 30):
            message += f"本月尚未更新，上次更新日期為 {artist[2]}\n"
          elif artist[4] == 0:  message += '本月已更新\n'
          elif artist[4] == 2:  message += '本月繪師停更\n'
          message += f'預覽：{subscriber[1]}\n下載：{subscriber[2]}'
          
          await ctx.send(message, hidden=True)
        else:  await ctx.send("無人訂閱此繪師", delete_after = 5)
    else:  await ctx.send("頻道錯誤", delete_after = 5)
  ...
  async def upload(self, ctx, artist = "", subscriber = ""):
    if subscriber != "":  subscriber = subscriber.mention.replace("!", "")  #有指定訂閱者
    else:  subscriber = ctx.author.mention.replace('!', '')
