...
SAO = re.compile(
    "星[\~` ]*[爆暴]|c8763|star(\s)*burst|(10|十)(\s)*(秒|sec)|((星光)|(西瓜)|(\N{WATERMELON})) *([流榴][槤蓮連]) *[擊雞機(\N{ROOSTER})]|[桐銅]([谷古鼓][和核])?[人仁]|kirito|當我.?.?第[二2].|艾恩葛朗特"
)
            ...
            if hasattr(Errors, error_command):
                ...
                if not await self.bot.is_owner(ctx.author):
                    await self.bot.get_channel(812628283631206431).send(
                        f"指令：{ctx.message.content}(by {ctx.author.mention})\n錯誤：{error}"
                    )
