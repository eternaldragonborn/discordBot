	...
	async def on_message(self, msg):
		if msg.author != self.bot.user:
			if re.search(r'[rR啊阿ㄚ]{4,}', msg.content):
				...
				cat = str(random.choices(url["loadingCat"], weights = weights["loadingcat"])).strip("[]'")
				...
				pic = str(random.choices(url["emmm"], weights = weights["emmm"])).strip("[]'")
				await msg.channel.send(pic, delete_after = 5)
