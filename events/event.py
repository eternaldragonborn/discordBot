...
SAO = re.compile('星(爆|暴)|c8763|star(\s)*burst|(10|十)\s*(秒|sec)|(星光|西瓜|\N{WATERMELON}) *((流|榴)(槤|蓮|連)) *(擊|雞|機|\N{ROOSTER})|桐((谷|古)(和|核))?(人|仁)|kirito|當我.?.?第(二|2).')
...
class Events(Cog_Ext):
  ...
  async def on_message(self, msg):
    if msg.author != self.bot.user and not msg.author.bot:
      ...
      elif SAO.search(msg.content.lower()) or "<:098:719589717955575879>" in msg.content:
